...
class Parser:
    ...
    def p_for_statement(self, p):
        'for_statement : FOR LPAREN assignment condition SEMICOLON for_increment RPAREN statement'
        print("Parsing: for_statement -> for (assignment; condition; increment) statement")
        p[0] = ('for', p[3], p[4], p[6], p[8])
    
    def p_for_increment(self, p):
        'for_increment : ID ASSIGN expression'
        print(f"Parsing: for_increment -> {p[1]} = expression")
        p[0] = ('assign', p[1], p[3])
